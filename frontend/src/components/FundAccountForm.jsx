...
const FundAccountForm = ({ accounts, onSubmit, loading }) => {
  ...
  return (
    <div className="bg-card-bg border border-gold/20 rounded-xl p-6">

      <div className="flex items-center gap-2 mb-4">
        <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-gold/10 border border-gold/20">
          <div className="w-1.5 h-1.5 rounded-full bg-gold" />
          <span className="font-mono text-xs text-gold">SYSTEM</span>
        </div>
        <h2 className="text-sm font-semibold text-cream">Fund account</h2>
      </div>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        ...
        {showPreview && (
          <div className="bg-dark-bg border border-border-subtle rounded-lg p-3 flex items-center justify-between">
